...
def generate_visualizations(df, tab=None):
    ...
    if tab == "Geographic Data":
        ...
        if zip_columns:
            ...
            # These lines are where the changes should be made:
            ...
            size_col = st.sidebar.selectbox("Size by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key=f"{tab}_geo_size_col_GEO")  
            color_col = st.sidebar.selectbox("Color by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key=f"{tab}_geo_color_col_GEO")  
    if tab == "KPI":
        filterable_cols = df.select_dtypes(include=['object']).columns.tolist()
