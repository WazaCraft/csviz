...
def generate_visualizations(df, tab=None):
    ...
    if zip_columns:
        ...
        filter_col = st.sidebar.selectbox("Filter by:", filterable_cols, index=default_filter_index, key="geo_filter_col_GEO")  # Updated key here
        filter_value = st.sidebar.selectbox("Select value:", geo_df[filter_col].unique(), key="geo_filter_value_GEO")  # Updated key here

        filtered_geo_df = geo_df[geo_df[filter_col] == filter_value]

        size_col = st.sidebar.selectbox("Size by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key="geo_size_col_GEO")  # Updated key here
        color_col = st.sidebar.selectbox("Color by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key="geo_color_col_GEO")  # Updated key here
