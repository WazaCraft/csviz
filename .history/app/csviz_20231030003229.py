...
def generate_visualizations(df, tab=None):
    ...
    if tab == "Geographic Data":
        ...
        filterable_cols = df.select_dtypes(include=['object']).columns.tolist()
        filter_col = st.sidebar.selectbox("Filter KPIs by:", filterable_cols, index=filterable_cols.index('Customer Segment'), key=f"{tab}_filter_col")
        filter_val = st.sidebar.selectbox("Select value:", df[filter_col].unique(), key=f"{tab}_filter_val")
