...
def generate_visualizations(df, tab=None):
    if tab is None:  # If tab is not passed, use the sidebar selectbox
    ...
    if tab == "Geographic Data":
        ...
        filter_val = st.sidebar.selectbox("Select value:", df[filter_col].unique(), key="kpi_filter_value")
        filtered_kpi_df = df[df[filter_col] == filter_val]
        
        # Ensure that the filtered data is used to calculate the KPIs
