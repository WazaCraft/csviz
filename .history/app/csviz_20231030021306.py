...
def generate_visualizations(df, tab=None):
    ...
    if tab == "Geographic Data":
        ...
        if zip_columns:
            ...
            filter_value = st.selectbox("Select value:", geo_df[filter_col].unique(), key="geo_filter_value_GEO")

            filtered_geo_df = geo_df[geo_df[filter_col] == filter_value]

            size_col = st.selectbox("Size by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key="geo_size_col_GEO")
