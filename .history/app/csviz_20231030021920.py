...
def generate_visualizations(df, tab=None):
    filterable_cols = df.select_dtypes(include=['object']).columns.tolist()  # Define filterable_cols here
    if tab is None:
    ...
    if tab == "Geographic Data":
        ...
        if zip_columns:
            ...
            size_col = st.selectbox("Size by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key="geo_size_col")
            color_col = st.selectbox("Color by:", df.select_dtypes(include=['float64', 'int64']).columns.tolist(), key="geo_color_col")
