...
def generate_visualizations(df, tab=None, kpi_filter_col=None, kpi_filter_val=None):
    if tab is None:
    ...
    if tab == "Geographic Data":
        ...
        key="kpi_filter_col_KPI"  # Unique key
)

        filter_val = st.sidebar.selectbox("Select value:", df[filter_col].unique(), key="kpi_filter_val")

        filtered_kpi_df = df[df[kpi_filter_col] == kpi_filter_val]
...
if df is not None:
    ...
    with tab5:
        generate_visualizations(df, tab='KPI', kpi_filter_col=filter_col, kpi_filter_val=filter_val)
